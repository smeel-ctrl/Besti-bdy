...
function askLoveQuestion() {
  Swal.fire({
    ...
    customClass: {
      title: "love-title",
      confirmButton: "yes-btn",
      cancelButton: "no-btn",
      popup: "love-popup"
    },

    didOpen: () => {
      const yesBtn = Swal.getConfirmButton();
      const noBtn = Swal.getCancelButton();

      const moveNoButton = () => {
        const yesRect = yesBtn.getBoundingClientRect();
        const popupRect = Swal.getPopup().getBoundingClientRect();
        ...
        do {
          const maxX = popupRect.width / 2 - noBtnRect.width - 16;
          const maxY = popupRect.height / 2 - noBtnRect.height - 16;
        ...
        if (attempts === MAX_ATTEMPTS) {
          noBtn.style.transform = "translate(0, 110px)";
        }
      };
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const targetDay = 2;
  const targetMonthIndex = 1; // December
